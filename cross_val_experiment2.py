...
def get_files_from_path(namepath: NamePath):
    """Get files from path"""
    data = os.path.join(namepath.path, f"df_{namepath.name}.csv")
    labels = os.path.join(namepath.path, f"label_{namepath.name}.csv")
    posidx = os.path.join(namepath.path, f"gene_idx_{namepath.name}.csv")
    return DataPaths(namepath.name, data, labels, posidx)


def get_all_dataset_files(basic_path: str):
    data_paths = [get_files_from_path(i) for i in get_datasets_paths(prm.DATA_PATH)]
    # cwd = os.getcwd()
    for data_path in data_paths:
        assert os.path.exists(
            os.path.join(basic_path, data_path.name)
        ), f"directory: {data_path.name} does not exist"
        assert os.path.exists(data_path.data), f"file: {data_path.data} does not exist"
        assert os.path.exists(
            data_path.labels
        ), f"file: {data_path.labels} does not exist"
        assert os.path.exists(
            data_path.posidx
        ), f"file: {data_path.posidx} does not exist"
    return data_paths


def dataframe_from_csv_preproc(*, csv_path: str, transpose: bool):
    """Read dataframe from csv file and preprocess it"""
    dfr = pd.read_csv(csv_path, index_col=0)
    if transpose:
        dfr = dfr.T
...
def labels_from_csv_preproc(csv_path: str):
    ...
    labels_unique_lst = list(labels_df["x"].unique())
    label2index = {name: i for i, name in enumerate(labels_unique_lst)}
    index2label = {j: i for i, j in label2index.items()}
    labels_df.replace(label2index, inplace=True)
    labels_df = labels_df.rename(columns={"x": "labels"})
    # print(list(labels["labels"]))
    return LabelDfDic(labels_df, label2index, index2label)
...
def feature_index_from_csv(*, path: str, keep_first: int = 0):
    ...
    """
    feature_index_df = pd.read_csv(path)
    method_names = list(feature_index_df.columns)
    fs_dic = {"full": []}  # {method_name: [2048, 1035, 2442, 45324, ...]}
    for name in method_names:
        gene_index = pd.DataFrame(feature_index_df[name])
...
def workflow(data_paths: list, repeats=prm.REPEATS):
    ...
    for dpath in data_paths:
        ...
        alg_best_features = feature_index_from_csv(
        ...
        for algorithm in CLASSIFIERS:
            ...
            results[algorithm] = {}
            for findex, vals in alg_best_features.items():
                ...
                for i in range(repeats):
                    msg(f"{dpath.name} {algorithm} {findex} repeat: {i + 1}")
...
def main():
    ...
    data_paths = get_all_dataset_files(prm.DATA_PATH)
    workflow(data_paths)
...
if __name__ == "__main__":
    main()
    # data_paths = get_datasets_paths("./data")
    # print(get_all_dataset_files(prm.DATA_PATH))
